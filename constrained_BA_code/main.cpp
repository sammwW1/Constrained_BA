...
Eigen::Quaterniond first_frame_sensor,Eigen::Quaterniond second_frame_sensor,const double remove_percent);
...
{   
    ...
    {   
        ...
        tie(our_e[i], sensor_e[i]) = returnTheRotationAngle(img1,img2,pts1,pts2,first_frame,second_frame, 0);
       
    }
    vector<double> our_e_sub = {our_e.begin(), our_e.end() - 1};
    vector<double> sensor_e_sub = {sensor_e.begin(), sensor_e.end() - 1};

    // plot///////////////////////////////
    // normorlize
    vector<double> our_e_sub2= {our_e.begin(), our_e.end() - 1};
    vector<double> sensor_e_sub2= {our_e.begin(), our_e.end() - 1};
    double mod = 0.0;
    double mod2 = 0.0;

    for (size_t i = 0; i < our_e_sub.size(); ++i) {
        mod += our_e_sub[i] * our_e_sub[i];
        mod2 += sensor_e_sub[i] * sensor_e_sub[i];
    }

    double mag = std::sqrt(mod);
    double mag2 = std::sqrt(mod2);


    for (size_t i = 0; i < our_e_sub.size(); ++i) {
        our_e_sub2[i] = our_e_sub[i] / mag;
        sensor_e_sub2[i] = sensor_e_sub[i] / mag2;
    }
    plt::figure_size(1200, 780);
    // Plot line from given x and y data. Color is selected automatically.
    plt::plot(frames, our_e_sub2);
    // Plot a red dashed line from given x and y data.
    plt::plot(frames, sensor_e_sub2,"r--");
    plt::show();
...
Eigen::Quaterniond first_frame_sensor,Eigen::Quaterniond second_frame_sensor, const double remove_percent)
{
    ...
    // linear solver in CHolmod linear
    ...
    // ? 6DOF pose + 3DOF point = 9 variable dimension
    // BlockSolverTraits<a,b> a and b refer to the dimension of the vertices that are part of certain blocks when computing the Schur complement.
    ...
    first_camera->setId(0);
    g2o::SE3Quat frame_one_measure;
    frame_one_measure.setRotation(first_frame_sensor);
    frame_one_measure.normalizeRotation();
    first_camera->setEstimate(frame_one_measure);
    first_camera->setFixed(true);
    optimizer.addVertex(first_camera);
    
    g2o::VertexSE3Expmap *second_camera = new g2o::VertexSE3Expmap();
    second_camera->setId(1);
    g2o::SE3Quat frame_two_measure;
    frame_two_measure.setRotation(second_frame_sensor);
    frame_two_measure.normalizeRotation();
    second_camera->setEstimate(frame_two_measure);
    ...
    Eigen::Matrix<double, 1, 1> info(pts1.size()*10);
    ...
    g2o::VertexSE3Expmap *result = dynamic_cast<g2o::VertexSE3Expmap *>(optimizer.vertex(1));
    
    

    // our estimation
    g2o::Quaternion our_estimation = result->estimate().rotation();
    g2o::Quaternion init_estimation = first_camera->estimate().rotation();
    Eigen::Matrix3d our_estimation_matix = our_estimation.normalized().toRotationMatrix();
    Eigen::Matrix3d init_estimation_matix = init_estimation.normalized().toRotationMatrix();
    Eigen::Matrix3d temp = init_estimation_matix * our_estimation.inverse();
    AngleAxis axis_temp(temp);
    double output = axis_temp.angle();


    optimizer.clear();
    return  std::make_tuple(output, relative_extimation);
    //return  std::make_tuple(average_error, percent_inlier);
